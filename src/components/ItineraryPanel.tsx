...
function WeatherChip({ forecast }: { forecast?: WeatherSnapshot }) {
  ...
  return (
    <div className="flex items-center gap-1 rounded-full border border-line bg-card px-2 py-1 text-xs font-medium text-ink/70">
...
function LocalPulse({ trip }: { trip: Trip }) {
  ...
  return (
    <div className="mb-5 flex items-center gap-2 rounded-xl border border-line bg-card px-3 py-2.5">
      <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-teal/10 text-teal">
        <Radio className="h-3.5 w-3.5 animate-pulse" />
      </span>
      <div className="min-w-0">
        <div className="text-[10px] font-semibold uppercase tracking-wide text-teal">Live in {trip.destination.split(",")[0]}</div>
...
export function ItineraryPanel({ trip, onSelectPlace, onBookStay, onSwapPlace, onRemovePlace }: ItineraryPanelProps) {
  ...
  return (
    <div className="scrollbar-thin h-full overflow-y-auto px-4 py-5">
      <div className="mb-5">
        <div className="text-[11px] font-semibold uppercase tracking-wide text-ink/50">Itinerary</div>
