...
export function MessageBubble({ message, onSelectPlace, onBookStay }: MessageBubbleProps) {
  ...
  if (isUser) {
    return (
      <div className="flex justify-end">
        <div className="max-w-[85%] rounded-2xl rounded-br-sm bg-ink px-4 py-3 text-sm text-parchment sm:max-w-[70%]">
          {message.text}
        </div>
      </div>
    );
  ...
  return (
    <div className="flex items-start gap-2.5">
      <div className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-ink text-brass">
      ...
      <div className="flex min-w-0 max-w-[90%] flex-col gap-3 sm:max-w-[75%]">
        <div
          className={`rounded-2xl rounded-tl-sm px-4 py-3 text-sm ${
            isError
              ? "border border-brass-dark/40 bg-brass/10 text-ink"
              : "bg-card text-ink shadow-sm"
          }`}
        >
          {isError && (
            <div className="mb-1.5 flex items-center gap-1.5 font-mono text-xs font-semibold uppercase tracking-wide text-brass-dark">
