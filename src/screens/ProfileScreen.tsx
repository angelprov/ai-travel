...
function Field({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between border-b border-hairline py-3 last:border-b-0">
      <span className="font-mono text-xs uppercase tracking-wide text-ink/50">{label}</span>
...
export function ProfileScreen() {
  ...
  return (
    <div className="scrollbar-thin h-full overflow-y-auto px-5 py-6" style={{ paddingTop: "calc(1.5rem + env(safe-area-inset-top))" }}>
      ...
      <div className="mb-6 flex items-center gap-3 rounded-2xl border border-hairline bg-card p-4">
        <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-ink text-brass font-display text-lg">
          {(profile.name || user?.email || "?").charAt(0).toUpperCase()}
        </div>
        <div className="min-w-0">
          <div className="truncate text-sm font-medium text-ink">{profile.name || "Traveler"}</div>
          <div className="flex items-center gap-1 truncate font-mono text-xs text-ink/50">
      ...
      <div className="mb-6 rounded-2xl border border-hairline bg-card px-4">
      ...
      <button
        ...
        className="flex w-full items-center justify-center gap-2 rounded-full border border-hairline bg-card py-3 text-sm font-semibold text-ink transition-colors hover:bg-ink/5"
      >
